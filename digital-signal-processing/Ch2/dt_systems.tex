...
\subsection{Shift-Invariant vs Shift-Variant}
A DT system is said to be \emph{shift-invariant} if a shift in the input signal results in a corresponding shift in the output signal such that $\Psi[x[n-N]]=y[n-N]$. That is:
\begin{center}
    \begin{tikzpicture}
    ...
    \end{tikzpicture}
\end{center}
Some resources refer to this property for DT signals as ``time-invariant'' as each sample represents an instance in time; here, we will use the more general term 
``shift-invariant'' to highlight the sampled nature of the signals.
\\ \\
